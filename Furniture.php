...
<body>
    <section class="fun-items">
        <div class="class1">
        <h1 id="jk">Furniture</h1>
    </div>
    <div class="tabs">
    <input type="radio" class="tabs__radio" id="tab1" name="tabs_group" value="SofaSets" checked>
    <label for="tab1" class="tabs__label">Sofa Set </label>

    <div class="all-fun-items">
            <?php
            @include 'config.php';
            $select_products = mysqli_query($conn, "SELECT * FROM `products` WHERE type='sofa'");
      ...
      ?>
        ...
        <label for="tab2" class="tabs__label">Tables </label>
        <div class="all-fun-items">
            <?php
            @include 'config.php';
            $select_products = mysqli_query($conn, "SELECT * FROM `products` WHERE type='table'");
            if(mysqli_num_rows($select_products) > 0){
                while($fetch_product = mysqli_fetch_assoc($select_products)){
            ?>
            <div class="item">
    ...
    <label for="tab3" class="tabs__label">Bed Room </label>

    <div class="all-fun-items">
            <?php
            @include 'config.php';
            $select_products = mysqli_query($conn, "SELECT * FROM `products` WHERE type='bed'");
            if(mysqli_num_rows($select_products) > 0){
                while($fetch_product = mysqli_fetch_assoc($select_products)){
            ?>
            <div class="item">
